...
class RelationGraphProjector(nn.Module):
    """Masked cross-attention pooler for one causal relation branch."""

    def __init__(self, config):
        super().__init__()
        self.hidden_dim = int(getattr(config, "hidden_size", 4096))
        self.num_queries = int(getattr(config, "relation_graph_tokens", 4))
        self.input_proj = nn.Sequential(
            nn.Linear(config.mm_hidden_size, self.hidden_dim),
            nn.GELU(),
            nn.Linear(self.hidden_dim, self.hidden_dim),
        )
        self.queries = nn.Parameter(torch.empty(self.num_queries, self.hidden_dim))
        self.null_tokens = nn.Parameter(torch.empty(self.num_queries, self.hidden_dim))
    ...
    def forward(self, x, valid_mask=None):
        if x.ndim != 3:
            raise ValueError(f"relation graph embeddings must be [B,N,D], got {tuple(x.shape)}")
        ...
        scores = torch.einsum("bnh,qh->bqn", projected, self.queries)
        scores = scores / math.sqrt(self.hidden_dim) / self.temperature.clamp_min(1e-4)
        ...
        output = torch.einsum("bqn,bnh->bqh", weights, projected)
        if empty.any():
            output = output.clone()
            output[empty] = self.null_tokens.to(dtype=output.dtype)
        return output
...
class GraspMetaModel:
    ...
    def __init__(self, config):
        ...
        if hasattr(config, "mm_hidden_size"):
            ...
            if getattr(config, "dual_graph_projector", False):
                self.user_graph_projector = RelationGraphProjector(config)
                self.business_graph_projector = RelationGraphProjector(config)
    ...
    def initialize_graph_modules(self, model_args, fsdp=None):
        ...
        self.config.dual_graph_projector = getattr(model_args, 'dual_graph_projector', False)
        self.config.relation_graph_tokens = getattr(model_args, 'relation_graph_tokens', 4)


        self.mm_projector = build_graph_projector(self.config)
        if self.config.dual_graph_projector:
            self.user_graph_projector = RelationGraphProjector(self.config)
            self.business_graph_projector = RelationGraphProjector(self.config)
        ...
        if pretrain_mm_mlp_adapter is not None:
            ...
            if self.config.dual_graph_projector:
                user_weights = get_w(mm_projector_weights, 'user_graph_projector')
                business_weights = get_w(mm_projector_weights, 'business_graph_projector')
                if user_weights:
                    self.user_graph_projector.load_state_dict(user_weights)
                if business_weights:
                    self.business_graph_projector.load_state_dict(business_weights)
...
class GraspMetaForCausalLM(ABC):
    ...
    def encode_graphs(self, graph, graph_emb, graph_types=None):
        valid_mask = graph != DEFAULT_GRAPH_PAD_ID
        graph_emb = graph_emb.masked_fill(~valid_mask.unsqueeze(-1), 0.0)
        model = self.get_model()
        if not getattr(self.config, "dual_graph_projector", False):
            return list(model.mm_projector(graph_emb))

        if graph_types is None or len(graph_types) != graph.shape[0]:
            raise ValueError(
                f"dual graph mode requires one graph marker per graph: "
                f"types={0 if graph_types is None else len(graph_types)} graphs={graph.shape[0]}"
            )
        features = []
        for row, marker in enumerate(graph_types):
            x = graph_emb[row:row + 1]
            mask = valid_mask[row:row + 1]
            if marker == USER_GRAPH_TOKEN_INDEX:
                encoded = model.user_graph_projector(x, mask)
            elif marker == BUSINESS_GRAPH_TOKEN_INDEX:
                encoded = model.business_graph_projector(x, mask)
            elif marker == GRAPH_TOKEN_INDEX:
                encoded = model.mm_projector(x)
            else:
                raise ValueError(f"unknown graph marker index: {marker}")
            features.append(encoded[0])
        return features

    @staticmethod
    def _graph_marker_positions(input_ids):
        mask = torch.zeros_like(input_ids, dtype=torch.bool)
        for marker in GRAPH_TOKEN_INDICES:
            mask |= input_ids == marker
    ...
    def prepare_inputs_labels_for_multimodal(
        ...
        if graphs is None or graph_emb is None:
        ...
        graph_types = [
            ...
            if int(token) in GRAPH_TOKEN_INDICES
        ]
        graph_features = self.encode_graphs(graphs, graph_emb, graph_types)

        new_input_embeds = []
        new_labels = [] if labels is not None else None
        cur_graph_idx = 0
        for batch_idx, cur_input_ids in enumerate(input_ids):
            if self._graph_marker_positions(cur_input_ids).numel() == 0:
                ...
                cur_graph_idx += 1
                continue
            graph_token_indices = self._graph_marker_positions(cur_input_ids)
