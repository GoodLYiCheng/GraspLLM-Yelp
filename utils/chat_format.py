...
USER_GRAPH_PLACEHOLDER_CHAR = "\uE001"
BUSINESS_GRAPH_PLACEHOLDER_CHAR = "\uE002"
_GRAPH_PLACEHOLDERS = {
    DEFAULT_GRAPH_TOKEN: (GRAPH_PLACEHOLDER_CHAR, GRAPH_TOKEN_INDEX),
    DEFAULT_USER_GRAPH_TOKEN: (USER_GRAPH_PLACEHOLDER_CHAR, USER_GRAPH_TOKEN_INDEX),
    DEFAULT_BUSINESS_GRAPH_TOKEN: (BUSINESS_GRAPH_PLACEHOLDER_CHAR, BUSINESS_GRAPH_TOKEN_INDEX),
}
...
def _locate_placeholders_via_text(tokenizer, msgs, add_generation_prompt: bool
    ...
    positions = []
    char_to_index = {placeholder: index for placeholder, index in _GRAPH_PLACEHOLDERS.values()}
    for offset, char in enumerate(text):
        if char in char_to_index:
            positions.append((offset, char_to_index[char]))
    return text, positions


def _replace_graph_tokens(content: str) -> str:
    for token, (placeholder, _) in _GRAPH_PLACEHOLDERS.items():
...
def build_supervised(tokenizer,
    ...
    for turn in conversations:
        ...
        if has_graph and role == "user":
            content = _replace_graph_tokens(content)
    ...
    if has_graph:
        ...
        new_mask: List[int] = []
        consumed_token = 0
        for char_idx, graph_token_index in char_positions:
...
def build_multi_turn_eval_prompt(tokenizer,
    ...
    for turn in conversations:
        role = role_map.get(turn["from"], turn["from"])
        content = _replace_graph_tokens(turn["value"]) if has_graph and role == "user" else turn["value"]
        msgs.append({"role": role, "content": content})
    ...
    if has_graph:
        ...
        new_ids: List[int] = []
        consumed = 0
        for char_idx, graph_token_index in positions:
            target_len = len(text[:char_idx])
            lo, hi = consumed, len(ids)
            while lo < hi:
                mid = (lo + hi) // 2
                if len(tokenizer.decode(ids[:mid], skip_special_tokens=False)) < target_len:
                    lo = mid + 1
                else:
                    hi = mid
            token_start = lo
            lo, hi = token_start, len(ids)
            while lo < hi:
                mid = (lo + hi) // 2
                if len(tokenizer.decode(ids[:mid], skip_special_tokens=False)) < target_len + 1:
                    lo = mid + 1
                else:
                    hi = mid
            new_ids.extend(ids[consumed:token_start])
            new_ids.append(graph_token_index)
            consumed = lo
        new_ids.extend(ids[consumed:])
        ids = new_ids
    if max_length is not None and len(ids) > max_length:
        raise ValueError(
            f"few-shot prompt has {len(ids)} tokens, exceeding max_length={max_length}; "
            "do not left-truncate graph/support alignment"
        )
    return torch.tensor(ids, dtype=torch.long)
