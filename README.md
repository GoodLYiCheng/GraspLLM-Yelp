...
## 4. Temporal dual-relation YelpZip experiment

The isolated implementation under `experiments/grasp_dual_relation/` treats each
review as a TAG node and enforces a strict chronological contract:

* earliest 60% of complete timestamps: self-supervised GNN pre-training;
* next 10%: projector alignment / strict few-shot support pool;
* next 15%: validation; final 15%: held-out test;
* relation edges point from history to current, and DFS may only move to a
  strictly earlier timestamp. Equal-timestamp and future reviews are excluded.

Prepare and audit the full YelpZip graph (no model training):

```bash
python -m experiments.grasp_dual_relation.cli \
  --raw-path /path/to/yelpzip.csv
```

This writes the split/node-order manifest, bounded user/business CSR graphs,
balanced 1/5/10-shot support IDs, motif-diversity audit and leakage report to
`artifacts/grasp_dual_relation/yelpzip/`. Run a small real-data check with
`--max-rows 5000 --output-dir artifacts/grasp_dual_relation/smoke_5000`.

On the external GPU host, place the canonical Qwen3 text embedding at
`artifacts/grasp_dual_relation/yelpzip/qwen3_emb_x.pt`, then run:

```bash
RAW_PATH=/path/to/yelpzip.csv \
  EMBED_MODEL=/data/Qwen3-Embedding-8B \
  bash scripts/stage0_dual_prepare.sh

bash scripts/stage1_dual_gnn_pretrain.sh

RAW_PATH=/path/to/yelpzip.csv \
  bash scripts/stage2_dual_generate_contexts.sh

BASE_MODEL=/data/Qwen/Qwen3-8B \
  bash scripts/stage3_dual_projector_train.sh
```

The dual JSONL schema uses typed graph groups in prompt order:

```json
{"graphs": [
  {"type": "user", "nodes": [1, 2, -500]},
  {"type": "business", "nodes": [3, 4, -500]}
]}
```

Use `make_fewshot_data` to create strict K-shot projector/evaluation files and
`evaluate_llm` to score the canonical answers `Fraudulent` and `Legitimate` by
sequence likelihood. Select the classification threshold only with
`score_results --validation ... --test ...`.

The isolated `experiments/random/` package provides label-blind `uniform` and
historical `alignment_prior` difficulty baselines on the identical temporal
split. Its prediction JSONL is compatible with the paired comparison tool; see
`experiments/random/README.md` for full and fixed-20,000-query commands.

---

## 5. Static YelpZip zero-shot baseline

For the static, transductive YelpZip zero-shot base integrated into the original
Stage 0–3 pipeline, see `YELPZIP_BASE.md`. It runs RUR and RBR separately with a
shared Arxiv-trained projector and text-only node features.
