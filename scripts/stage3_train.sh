...
BS=8; GRAD_ACCUM=1; LR=5e-4; EPOCHS=1; WD=0.0; WARMUP=0.03
MAX_LEN=4096; PROJECTOR_TYPE="vicuna_2layermh"
BASE_MODEL=""; OUT_DIR=""; DEEPSPEED_CFG=""; CONV_OVERRIDE=""; PRECISION="auto"
...
# Ampere/Ada/Hopper support BF16; Volta (V100) does not.  ``auto`` keeps the
# original BF16 behaviour on A100+ and selects FP16 on V100 without edits.
if [[ "$PRECISION" == "auto" ]]; then
    CUDA_CAPABILITY=$(python - <<'PY'
import torch
print(torch.cuda.get_device_capability(0)[0] if torch.cuda.is_available() else 0)
PY
)
    if [[ "$CUDA_CAPABILITY" -ge 8 ]]; then
        PRECISION="bf16"
    else
        PRECISION="fp16"
    fi
...
case "$PRECISION" in
    bf16)
        ...
        ;;
    fp16)
        PRECISION_ARGS=(--fp16 True --bf16 False --tf32 False)
        ;;
    *)
        echo "--precision must be auto, bf16, or fp16 (got: $PRECISION)" >&2
        exit 1
        ;;
esac
...
echo "   gpus     : $GPUS  (nproc=$NPROC)  bs=$BS  grad_accum=$GRAD_ACCUM  lr=$LR  epochs=$EPOCHS"
echo "   max_len  : $MAX_LEN   precision=$PRECISION  deepspeed=${DEEPSPEED_CFG:-<off, plain DDP>}"
echo "   out      : $OUT_DIR"
echo "================================================================="
