...
def reuse_yelpzip_embedding(*, overwrite: bool = False) -> None:
    ...
    target_data = os.path.join(target_dir, "processed_data.pt")
    if not all(os.path.isfile(path) for path in (source_emb, source_data, target_data)):
        return
    left = _safe_torch_load(source_data)
    right = _safe_torch_load(target_data)
    left_meta = getattr(left, "metadata", {})
...
def main():
    ...
    ap.add_argument(
        "--attn-implementation", choices=("auto", "eager", "sdpa", "flash_attention_2"),
        default=os.environ.get("QWEN3_ATTN_IMPLEMENTATION", "auto"),
        help="Attention backend; auto uses eager on pre-Ampere GPUs such as V100.",
    )
    ap.add_argument(
        "--dtype", choices=("auto", "float16", "bfloat16"),
        default=os.environ.get("QWEN3_EMB_DTYPE", "auto"),
        help="Model dtype; auto selects float16 on V100 and bfloat16 on Ampere+.",
    )
    args = ap.parse_args()
    ...
    if args.attn_implementation == "auto":
        ...
        if major < 8:
            ...
            attn_implementation = "eager"
            print("[init] flash_attn unavailable; falling back to eager attention")
    else:
        attn_implementation = args.attn_implementation
    print(f"[init] dtype={model_dtype} attention={attn_implementation}")
    ...
    if "yelpzip_rur" in names and args.num_shards == 1:
        reuse_yelpzip_embedding(overwrite=args.overwrite)
