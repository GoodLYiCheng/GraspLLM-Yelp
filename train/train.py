...
from utils.constants import (
    DEFAULT_BUSINESS_GRAPH_TOKEN,
    DEFAULT_GRAPH_END_TOKEN,
    DEFAULT_GRAPH_PAD_ID,
    DEFAULT_GRAPH_START_TOKEN,
    DEFAULT_GRAPH_TOKEN,
...
class ModelArguments:
    ...
    pretrained_projector_path: Optional[str] = field(default=None)
    ...
    dual_graph_projector: bool = field(default=False)
    relation_graph_tokens: int = field(default=4)


def load_pretrained_mm_projector(model, path: str) -> None:
    """Load a single-graph projector exactly, refusing partial/random initialization."""
    if not os.path.isfile(path):
        raise FileNotFoundError(f"pretrained projector not found: {path}")
    state = torch.load(path, map_location="cpu", weights_only=False)
    if not isinstance(state, dict):
        raise ValueError("pretrained projector must be a state-dict file")
    target = model.state_dict()
    selected = {
        key: value for key, value in state.items()
        if "mm_projector" in key and key in target and tuple(value.shape) == tuple(target[key].shape)
    }
    expected = {key for key in target if "mm_projector" in key}
    missing = sorted(expected - set(selected))
    if not selected or missing:
        raise ValueError(
            "pretrained projector does not exactly match the initialized single-graph projector; "
            f"matched={len(selected)}, missing={missing[:5]}"
        )
    model.load_state_dict(selected, strict=False)
    rank0_print(f"Loaded {len(selected)} projector tensors from {path}")
...
class DataArguments:
    ...
    graph_embedding_path: Optional[str] = field(default=None)
...
def safe_save_model_for_hf_trainer(trainer: transformers.Trainer,
    ...
    if getattr(trainer.args, "tune_mm_mlp_adapter", False):
        # Only save Adapter
        if getattr(trainer.model.config, "dual_graph_projector", False):
            keys_to_match = ['user_graph_projector', 'business_graph_projector']
        else:
...
class LazySupervisedGraphDataset(Dataset):
    ...
    def __init__(self,
        ...
        if data_args.data_path:
            if not data_args.graph_embedding_path:
                raise ValueError("--graph_embedding_path is required with --data_path")
            with open(data_args.data_path, "r", encoding="utf-8") as handle:
                self.list_data_dict = [json.loads(line) for line in handle if line.strip()]
            for sample in self.list_data_dict:
                sample.setdefault("dataset", "yelpzip_grasp")
            embedding_obj = torch.load(
                data_args.graph_embedding_path, map_location="cpu", weights_only=False
            )
            embedding = embedding_obj["emb"] if isinstance(embedding_obj, dict) else embedding_obj
            self.pretrained_embs = {"yelpzip_grasp": embedding.float()}
            self.datas = {}
            self.structure_emb = None
            self.use_dataset = ["yelpzip_grasp"]
            self.use_task = ["nc"]
            self.tokenizer = tokenizer
            self.data_args = data_args
            rank0_print(f"Direct dual-relation data: {data_args.data_path}, size {len(self.list_data_dict)}")
            return
        ...
        for d, dataset in enumerate(self.use_dataset):
            ...
            if "arxiv" in dataset:
                ...
                ds_key = "sportsfit"
            elif "yelpzip_grasp" in dataset:
                ds_key = "yelpzip_grasp"
            else:
    ...
    def lengths(self):
        ...
        for sample in self.list_data_dict:
            if 'graphs' in sample:
                graph_token_size = sum(len(spec['nodes']) for spec in sample['graphs'])
            else:
                graph_token_size = len(sample.get('graph', []))
    ...
    def modality_lengths(self):
        ...
        for sample in self.list_data_dict:
            ...
            cur_len = cur_len if ('graph' in sample or 'graphs' in sample) else -cur_len
    ...
    def __getitem__(self, i) -> Dict[str, torch.Tensor]:
        ...
        has_graph = 'graph' in sample or 'graphs' in sample
        ...
        if has_graph:
            if 'graphs' in sample:
                ...
                for token, graph_type in (
                    (DEFAULT_USER_GRAPH_TOKEN, 'user'),
                    (DEFAULT_BUSINESS_GRAPH_TOKEN, 'business'),
                    (DEFAULT_GRAPH_TOKEN, 'generic'),
                ):
                    start = 0
                    while True:
                        position = prompt.find(token, start)
                        if position < 0:
                            break
                        expected.append((position, graph_type))
                        start = position + len(token)
                expected_types = [graph_type for _, graph_type in sorted(expected)]
                actual_types = [str(spec['type']) for spec in specs]
                if expected_types != actual_types:
                    raise ValueError(
                        f"graph marker order {expected_types} does not match graph specs {actual_types}"
                    )
                node_lists = [list(map(int, spec['nodes'])) for spec in specs]
                widths = {len(nodes) for nodes in node_lists}
                if len(widths) != 1:
                    raise ValueError("all typed graph node lists in a sample must have equal length")
                graph = torch.LongTensor(node_lists)
            else:
                legacy_graph = sample['graph']
                if not isinstance(legacy_graph[0], list):
                    legacy_graph = [legacy_graph]
                graph = torch.LongTensor(legacy_graph)
            mask = graph != DEFAULT_GRAPH_PAD_ID
            masked_graph_emb = self.pretrained_embs[sample["dataset"]][graph[mask]]
...
def _train():
    ...
    model = _BackboneCls.from_pretrained(
        ...
        # Projector-only tuning still runs the entire frozen backbone in the
        # forward pass.  Loading it in fp32 makes an 8B model alone consume
        # about 32 GB, which guarantees OOM on V100 32 GB before a batch can
        # run.  Keep quantised loading unchanged; otherwise honour Trainer's
        # selected fp16/bf16 compute dtype at load time.
        **({"torch_dtype": compute_dtype} if training_args.bits == 16 else {}),
        **bnb_model_from_pretrained_args,
    )
    ...
    if model_args.pretrained_projector_path:
        if model_args.dual_graph_projector:
            raise ValueError("--pretrained_projector_path is only supported for the single <graph> projector")
        load_pretrained_mm_projector(model, model_args.pretrained_projector_path)
    ...
    if model_args.tune_mm_mlp_adapter:
        ...
        if model_args.dual_graph_projector:
            for module_name in ("user_graph_projector", "business_graph_projector"):
                for p in getattr(model.get_model(), module_name).parameters():
    ...
    if training_args.bits in [4, 8]:
        ...
        if model_args.dual_graph_projector:
            model.get_model().user_graph_projector.to(dtype=compute_dtype, device=training_args.device)
            model.get_model().business_graph_projector.to(dtype=compute_dtype, device=training_args.device)
